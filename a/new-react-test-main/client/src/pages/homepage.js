...
const Home = ()  => {
    ...
    const login = () => {
        if(user === 'STUDENT NAME' && pass === 'STUDENT CODE'){
    ...
    return (
        ...
        <div id="flex-container" className="flex flex-col h-screen bg-cyan-600 font-proxima-nova" >
            ...
            gap-8 shadow-lg border-neutral-500 w-1/4 h-2/3 mt-10 bg-white rounded mr-10">
                ...
                <div id="input-containers" className="mx-5 mt-3" >
                    <p className="text-[.5rem] font-semibold">{user}</p>
                    <input ref={userRef} type="text" placeholder={placename} className="border-2 rounded text-center w-full"/>

                    <p className="text-[.5rem] font-semibold mt-5">{pass}</p>
                    <input ref={codeRef} type="text" placeholder={placepass} className="border-2 rounded text-center w-full"/>

                    <input type="submit" placeholder="login" className="block mt-10 bg-neutral-700 text-white w-full h-10 font-semibold hover:bg-neutral-600 cursor-pointer" onClick={login}/>
                </div>
